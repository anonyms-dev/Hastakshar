...
def load_signature(file_path):
    ...
    # try:
        # Load only the 1st, 2nd, and last columns
    ...
    if modes.ndim == 2 and modes.shape[0] < modes.shape[1]:
        modes = modes.T
    # hht_pressure = extract_hht_features(signal)
    # print(hht_pressure)
    # exit()
...
# -------------------------
# Evaluate or predict new signature
# -------------------------
def predict_signature(model_path, signature_path, device):
    """
    Load saved model and predict genuine/forged label for a single signature file.
    """
    # === Load model checkpoint ===
    checkpoint = torch.load(model_path, map_location=device)
    input_dim = checkpoint.get("input_dim", 16)  # fallback
    model = TemporalTransformer(input_dim=input_dim).to(device)
    model.load_state_dict(checkpoint["model_state"])
    model.eval()

    # === Load and preprocess signature ===
    try:
        features = load_signature(signature_path)
    except Exception as e:
        print(f"❌ Error reading signature: {e}")
        return None

    x = torch.tensor(features, dtype=torch.float32).unsqueeze(0).to(device)  # [1, T, F]
    mask = torch.zeros((1, x.size(1)), dtype=torch.bool).to(device)

    with torch.no_grad():
        prob = model(x, mask=mask).item()

    label = "✅ Genuine" if prob > 0.5 else "⚠️ Forged"
    print(f"\nPrediction for '{os.path.basename(signature_path)}': {label} (prob={prob:.3f})")
    return prob, label
...
if __name__ == "__main__":
    ...
    model_path = "temporal_transformer_signature.pt"

    # Example: use any test file path here
    test_signature = "/Users/balaji.raok/Documents/online_signature_work/Test_sample/sign_u021_P3_02.txt"

    if os.path.exists(model_path) and os.path.exists(test_signature):
        predict_signature(model_path, test_signature, device)
    else:
        print("⚠️ Model or test file not found. Skipping prediction.")
