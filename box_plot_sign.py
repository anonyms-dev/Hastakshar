...
def segment_strokes(points, pen_status=None):
    """
    Segments signature into strokes.
    If pen_status is given (0=pen-up, 1=pen-down), split based on it.
    Otherwise, treat whole sequence as one stroke.
    """
    strokes = []
    if pen_status is None:
        strokes.append(points)
    else:
        current = []
        for p, s in zip(points, pen_status):
            if s == 1:  # pen down
                current.append(p)
            else:
                if current:
...
def stroke_features(strokes):
    """
    Extract stroke-level features: length, avg speed, angle variation.
    """
    lengths, speeds, angles = [], [], []
    for stroke in strokes:
        deltas = np.diff(stroke, axis=0)
        if len(deltas) == 0:
            continue
        length = np.sum(np.linalg.norm(deltas, axis=1))
        speed = np.mean(np.linalg.norm(deltas, axis=1))
        angle = np.std(np.arctan2(deltas[:, 1], deltas[:, 0]))
        lengths.append(length)
        speeds.append(speed)
        angles.append(angle)
    return lengths, speeds, angles


def plot_stroke_boxplots(signatures, titles=None):
    """
    Plot boxplots for stroke-level features across multiple signatures.
    """
    all_lengths, all_speeds, all_angles = [], [], []

    for sig in signatures:
        strokes = segment_strokes(sig)
        lengths, speeds, angles = stroke_features(strokes)
        all_lengths.extend(lengths)
        all_speeds.extend(speeds)
        all_angles.extend(angles)

    plt.figure(figsize=(10, 6))
    plt.boxplot([all_lengths, all_speeds, all_angles], labels=["Length", "Speed", "Angle Var"])
    plt.title("Stroke-Level Variation (Boxplots)")
    plt.ylabel("Value")
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.show()

def plot_boxplots_multiple_signatures(all_feats, signature_ids=None, save_path=None):
    """
    all_feats: list of (T_i-1, 4) arrays for each signature
    signature_ids: list of labels for signatures
    """
    n_signatures = len(all_feats)
    feature_names = ["dx", "dy", "speed", "angle"]

    fig, axes = plt.subplots(n_signatures, 4, figsize=(12, 3*n_signatures), sharey=False)
    ...
    for i, feats in enumerate(all_feats):
        for j in range(4):
            axes[i, j].boxplot(feats[:, j], patch_artist=True)
            axes[i, j].set_title(feature_names[j])
            if j == 0:
                axes[i, j].set_ylabel(f"Signature {signature_ids[i] if signature_ids else i+1}")

    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, dpi=150)
        print(f"Saved boxplot grid to {save_path}")
    plt.show()
...
def main():
    # ====== edit these paths ======
    genuine_folder = "/Users/balaji.raok/Documents/data_base/English/Balaji/"   # set to your folder of genuine signatures (x,y per row)
    test_folder = "/Users/balaji.raok/Documents/data_base/English/Balaji/"         # folder of signatures to visualize (genuine or forgery)
    ...
    genuine_files = genuine_files[::5]  # e.g. genuine_files[::3] to downsample
    # plot_stroke_boxplots(genuine_files)
    

    train_xy = [load_signature_xy(p) for p in genuine_files]
    train_feats = [extract_features(xy) for xy in train_xy]
    plot_boxplots_multiple_signatures(
    train_feats,
    signature_ids=[f"Sig{i+1}" for i in range(len(train_feats))],
    save_path="multi_signature_boxplots_english.png")
    exit()
