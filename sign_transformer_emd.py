...
from vmd_decomposition import VMD, read_txt_signal,emd
import torch
...
def load_signature(file_path):
    ...
    # print(signal)
    # exit()
    imf_signal= emd(np.asanyarray(signal), max_imfs=5)
    imf_signal = np.transpose(imf_signal)
    # Debug shapes
    # print(f"data: {data.shape}, gmm_hmm: {gmm_hmm_data.shape}, modes: {modes.shape}")
    print(np.shape(data), np.shape(gmm_hmm_data), np.shape(imf_signal))
    # exit()
    # Align all to same time length by trimming to min (safe)
    min_len = min(len(data), len(gmm_hmm_data), len(imf_signal))
    data = data[:min_len]
    gmm_hmm_data = gmm_hmm_data[:min_len]
    imf_signal = imf_signal[:min_len]
    # imf_signal = np.transpose(imf_signal)


    print(np.shape(data), np.shape(gmm_hmm_data), np.shape(imf_signal))
    # Concatenate features along feature axis
    temp_data = np.concatenate((data, gmm_hmm_data, imf_signal), axis=1)  # shape (T, F_total)
...
#     # --- Fix #1: Ensure all sequences have same feature dimension ---
#     feat_dims = [s.shape[1] for s in sequences]
#     min_feat_dim = min(feat_dims)
#     if len(set(feat_dims)) > 1:
#         print(f"⚠️ Feature mismatch detected: {feat_dims} — trimming to {min_feat_dim}")
#         sequences = [s[:, :min_feat_dim] for s in sequences]

#     # --- Fix #2: Ensure all lengths (T) match mask correctly ---
#     lengths = torch.tensor([s.shape[0] for s in sequences], dtype=torch.long)
#     max_len = lengths.max().item()
#     padded_sequences = pad_sequence(sequences, batch_first=True)  # [B, T_max, F]
#     mask = torch.arange(max_len)[None, :].to(lengths.device) >= lengths[:, None]
...
def collate_fn(batch):
    ...
    # --- Fix: ensure all sequences have same feature dimension ---
    feat_dims = [s.shape[1] for s in sequences]
    min_feat_dim = min(feat_dims)
    if len(set(feat_dims)) > 1:
        print(f"⚠️ Feature mismatch detected: {feat_dims} → trimming to {min_feat_dim}")
